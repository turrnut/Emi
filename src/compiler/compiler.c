...
static void emitByte(uint8_t byte) {
  writeIR(currentIR(), byte, parser.previous.line);
}
//< Compiling Expressions emit-byte
...
static void emitLoop(int loopStart) {
  ...
  int offset = currentIR()->count - loopStart + 2;
...
static uint8_t makeConstant(Value value) {
  int constant = addConstant(currentIR(), value);
...
static void patchJump(int offset) {
  ...
  int jump = currentIR()->count - offset - 2;

  if (jump > UINT16_MAX) {
    error("Too much code to jump over.");
  }

  currentIR()->code[offset] = (jump >> 8) & 0xff;
  currentIR()->code[offset + 1] = jump & 0xff;
}
//< Jumping Back and Forth patch-jump
...
/* Compiling Expressions dump-chunk < Calls and Functions disassemble-end
    disassembleIR(currentIR(), "code");
*/
//> Calls and Functions disassemble-end
    disassembleIR(currentIR(), function->name != NULL
...
//> for-increment
  if (!match(TOKEN_RIGHT_PAREN)) {
    ...
    int incrementStart = currentIR()->count;
    expression();
    emitByte(OP_POP);
    consume(TOKEN_RIGHT_PAREN, "Expect ')' after for clauses.");
...
bool compile(const char* source, IR* chunk) {
*/
